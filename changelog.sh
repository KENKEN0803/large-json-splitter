...
function display-release {
  ...
  function fmt:hash {
    ...
    md) printf '[`%s`](https://github.com/KENKEN0803/large-json-splitter/commit/%s)' "$hash" "$hash" ;;
    esac
  }
  ...
  function fmt:subject {
    ...
    md) sed -E 's|#([0-9]+)|[#\1](https://github.com/KENKEN0803/large-json-splitter/issues/\1)|g' <<< "$subject" ;;
    esac
  }
